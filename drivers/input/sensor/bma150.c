...
static ssize_t bma150_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
	...
	pr_info("BMA150: X axis: %d\n" , acc.x);
	pr_info("BMA150: Y axis: %d\n" , acc.y);
	pr_info("BMA150: Z axis: %d\n" , acc.z);
#endif
	return 0;
}
...
static ssize_t bma150_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
	if (bma150_client == NULL)
		return -1;
#if DEBUG
	pr_info("BMA150: deivce should be accessed with ioctl command\n");
#endif
	return 0;
}
...
static int bma150_open(struct inode *inode, struct file *file)
{
	if (bma150_client == NULL) {
#if DEBUG
	 	pr_err("BMA150: I2C driver not install\n");
#endif
		return -1;
	}
	...
	if (smb380.chip_id > 0)	{
#if DEBUG
		pr_info("BMA150: ChipId: 0x%x\n" , smb380.chip_id);
		pr_info("BMA150: ALVer: 0x%x MLVer: 0x%x\n", smb380.al_version, smb380.ml_version);
#endif
	} else {
#if DEBUG
...
#if DEBUG
	pr_info("BMA150: device has been opened\n");
#endif
	return 0;
}
...
{
#if DEBUG
	pr_info("BMA150: device has been closed\n");
#endif
	return 0;
}
...
static long bma150_unlock_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#if DEBUG
			pr_err("BMA150: copy_from_user error\n");
#endif
			return -EFAULT;
		}
...
#if DEBUG
			pr_err("BMA150: copy_to_user error\n");
#endif
			return -EFAULT;
		}
		return err;
	...
	default:
		pr_info("BMA150: default cmd setters");
		return 0;
	}
...
static int bma150_init_chip(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "Initialize chip");
...
int bma150_probe(struct i2c_client *client, const struct i2c_device_id * devid)
{
	...
	if (!(data = kzalloc(sizeof(struct bma150_data), GFP_KERNEL)))
	{
		err = -ENOMEM;
		goto exit;
	}
	...
	if (i2c_smbus_read_byte(client) < 0) {
		dev_err(dev, "i2c_smbus_read_byte error!!\n");
		goto exit_kfree;
	} else {
	...
	if ((tempvalue & 0x00FF) == 0x0002) {
		dev_info(dev, "BMA150/SMB380 registered I2C driver!\n");
	} else {
		dev_warn(dev, "BMA150/SMB380 not registered 0x%x!\n", tempvalue);
		bma150_client = NULL;
		goto exit_kfree;
	}
	...
	if (tempvalue < 0) {
		dev_err(dev, "i2c_smbus_write_byte_data failed\n");
		return -EIO;
	}
...
#if DEBUG
	dev_info(&client->dev, "remove driver\n");
#endif
	kfree(bma);
	bma150_client = NULL;
...
static int bma150_suspend(struct i2c_client *client, pm_message_t state) {
	...
	u8 v;
#if DEBUG
	dev_info(&client->dev, "going to suspend");
#endif
...
static int bma150_resume(struct i2c_client *client) {
	...
	u8 v;
#if DEBUG
	dev_info(&client->dev, "resume from suspend");
#endif
...
static int __init bma150_init(void)
{
	...
	dev_info(dev, "Driver Initialisation successful\n");
...
out:
	pr_err("BMA150: Driver Initialisation failed\n");
	return res;
}
